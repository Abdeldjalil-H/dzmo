...
class LessonsList(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            context['completed_chapters'] = self.request.user.progress.completed_chapters.filter(topic = topic)
            context['solved_exercices'] = self.request.user.progress.solved_exercices.filter(chapter__topic = topic)
            context['wrong_exercices']= [sol.exercice for sol in self.request.user.exercicesolution_set.filter(correct = False)]
        return context
...
class LessonDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['solved_exercices'] = self.request.user.progress.solved_exercices.filter(chapter = chapter)
        context['wrong_exercices']= [sol.exercice for sol in self.request.user.exercicesolution_set.filter(exercice__chapter = chapter)]
        return context 
...
def get_ex(category, CHOICES = None):
    if category == 'result':
        ...
        class ExerciceForm(forms.Form):
            question = forms.ChoiceField(choices = [(f'{k}',CHOICES[k]) for k in range(0,len(CHOICES))],
        ...
        class ExerciceForm(forms.Form):
            question = forms.MultipleChoiceField(choices = [(f'{k}',CHOICES[k]) for k in range(0,len(CHOICES))],
                                                widget=forms.CheckboxSelectMultiple,
                                                label='',required=False)
        return ExerciceForm

def exerciceview(request, pk, chapter_slug,**kwargs):
    ...
    if not request.user.is_authenticated or not chapter.has_access(request):
        return redirect(reverse('lessons:chapter',
                kwargs={'topic': chapter.topic,
    ...
    context = {
        ...
        'solved_exercices': request.user.progress.solved_exercices.filter(chapter = chapter),
        'wrong_exercices': [sol.exercice for sol in request.user.exercicesolution_set.filter(exercice__chapter = chapter)]        
        }
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            if not obj:
                obj = ExerciceSolution(student = request.user,
                                        exercice_id = pk)
            else:
                obj = obj.first()
            ans = ''
            ...
            if obj.correct:
                ...
                if(all(ex in request.user.progress.solved_exercices.all() for ex in ex_of_chapter)):
                    print('hi')
