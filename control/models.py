...
class Submissions(models.Model):
    problems_subs = models.ManyToManyField(
        ProblemSubmission, blank=True, related_name="+"
    )
    last_correct_subs = models.ManyToManyField(
        ProblemSubmission, blank=True, related_name="+"
    )
    tasks_subs = models.ManyToManyField(
        "tasks.TaskProblemSubmission", blank=True, related_name="+"
    )
    ...
    @classmethod
    def get_problems_subs_bis(cls, filters=None):
        return cls.objects.first().problems_subs.filter(filters)
    ...
    def update_last_correct(cls):
        cls.remove_correct_subs(
            *cls.objects.first().last_correct_subs.filter(
                submited_on__lt=timezone.now() - timedelta(days=7)
            )
        )
...
def init_submissions():
    if Submissions.objects.first():
        ...
        instance.save()
    instance.problems_subs.add(
        *ProblemSubmission.objects.filter(status__in=["submit", "comment"])
    )
    instance.last_correct_subs.add(
        *ProblemSubmission.objects.filter(
            correct=True, submited_on__gte=timezone.now() - timedelta(days=7)
        )
    )
